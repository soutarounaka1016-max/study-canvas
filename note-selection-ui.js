...
const selectionHint = $("#noteSelectionHint");
const selectionActions = $("#noteSelectionActions");
const deleteSelectionButton = $("#noteDeleteSelectionButton");
const colorOptions = dialog.querySelector(".note-color-options");
const widthControl = dialog.querySelector(".note-width-control");
...
$("#backToNoteGalleryButton").addEventListener("click", () => {
  commitTitle();
  if (save()) showGallery();
});
...
dialog.querySelectorAll("[data-note-tool]").forEach((button) => {
  button.addEventListener("click", () => selectTool(button.dataset.noteTool));
});
...
for (const name of ["selectstart", "contextmenu", "dblclick"]) {
...
function selectTool(nextTool) {
  const safeTool = ["pen", "eraser", "select"].includes(nextTool) ? nextTool : "pen";
  if (safeTool !== tool) selection.clear();
  tool = safeTool;
...
function render() {
  ...
  if (tool === "select") selection.drawOverlay(context);
  updateSelectionActions();
  emptyHint.hidden = drawing.strokes.length > 0 || Boolean(stroke);
}

function updateSelectionActions() {
  const bounds = tool === "select" ? selection.bounds : null;
  selectionActions.hidden = !bounds;
  if (!bounds) return;
  const left = Math.max(0, Math.min(BASE_WIDTH, bounds.maxX + 14));
  const top = Math.max(0, Math.min(BASE_HEIGHT, bounds.minY - 14));
  selectionActions.style.left = `${(left / BASE_WIDTH) * 100}%`;
  selectionActions.style.top = `${(top / BASE_HEIGHT) * 100}%`;
  selectionActions.style.transform = top < 80 ? "translate(-100%, 0)" : "translate(-100%, -100%)";
}

function updateSelectionHint() {
  selectionHint.textContent = selection.hasSelection
