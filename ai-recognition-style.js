...
style.textContent = `
.taskize-ai-panel{grid-template-columns:auto auto minmax(0,1fr)}
.taskize-ai-panel .taskize-ai-settings-button{border-color:#8b5cf6;background:#fff;color:#5b21b6}
.taskize-ai-panel .taskize-ai-privacy{grid-column:1/-1;color:#6b21a8;line-height:1.45}
...
@media(max-width:780px){.taskize-ai-panel{grid-template-columns:1fr 1fr}.taskize-ai-panel p{grid-column:1/-1}}
@media(max-width:520px){.taskize-ai-panel{grid-template-columns:1fr}.taskize-ai-panel .taskize-ai-privacy,.taskize-ai-panel p{grid-column:auto}.ai-token-row{grid-template-columns:1fr}.ai-settings-actions{flex-direction:column-reverse}.ai-settings-actions button{width:100%}}
`;
