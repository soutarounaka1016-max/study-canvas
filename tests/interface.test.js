...
test("週間目標が現在の手書きバックアップ対象外であることを明記する", () => {
  assert.match(html, /週間目標は現在の手書きJSONバックアップには含まれません/);
});
...
test("自由ノートが現在の手書きバックアップ対象外であることを明記する", () => {
  assert.match(html, /自由ノートは現在の手書きJSONバックアップには含まれません/);
});
